...
bool CMainRenderer::OnCreate( void ){
    ...
    if( ! CBmpDotMgr::OnCreate() ){
        LOGE( "@ CBmpDotMgr::OnCreate(): FAILED!!\n" );
        env_push_error_log( "[CBmpDotMgr::OnCreate()]に失敗しました。\n" );
        return( false );
    }
...
void CMainRenderer::DrawLog( void ){
    ...
    // BMP DOT PART USE
    sprintf( buf, "BDPD: %d/%d", CBmpDotPartData::GetAllocDataNumUse(), CBmpDotPartData::GetAllocDataNumMax() );
    CDrawCell::DrawLogAlign( buf );

    // BMP DOT USE
    sprintf( buf, "BDD: %d/%d", CBmpDotData::GetAllocDataNumUse(), CBmpDotData::GetAllocDataNumMax() );
    CDrawCell::DrawLogAlign( buf );
