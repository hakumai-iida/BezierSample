...
//----------------------------------
void CBezier::PutTexForWorkPath( float x0, float y0, int size, BYTE* pTex, BYTE pal, int cut, bool isAdjA, bool isAdjB ){
    int w = size;
    int h = size;
    ...
    for( int i=cut; i<(h-cut); i++ ){
        for( int j=cut; j<(w-cut); j++ ){
            // 調整指定[A]があれば中央を除外
            if( isAdjA ){
                if( i>=(h/2-2) && i<=(h/2+2) && j>=(w/2-2) && j<=(w/2+2) ){
                    continue;
                }
            }
...
// [Work] ワークパス用：点の出力
//----------------------------------
void CBezier::PutPointForWorkPath( float x0, float y0, stBEZIER_ANCHOR_POINT* pBAP ){
    BYTE pal = BEZIER_WP_POINT_PAL + pBAP->workPathPalOfsS;
    
    // 選択中であればサイズを大きく（※常時選択時は無視）
    int size;
    if( (pal&0x0F) == BEZIER_WP_PAL_VAL_FOR_SELECTED && !IsDispAllSelected() ){
        size = s_nSizeForWorkPathSelected;
    }else{
        size = s_nSizeForWorkPathPoint;
    }
    if( s_bTouch ){
        size = (2*size+2)/3;
    }
    // 奇数にする
    size = 2*(size/2)+1;
    PutTexForWorkPath( x0, y0, size, NULL, pal, 0, false, pBAP->isRefJoint );
    
    // フック／ジョイント点であれば見た目の調整
    if( pBAP->hookTargetIdFixed != eSTROKE_HOOK_TARGET_INVALID || pBAP->isJointed ){
        // ジョイント
        if( pBAP->isJointed ){
            pal = BEZIER_WP_HOOK_SYSTEM_POINT_PAL + pBAP->workPathPalOfsS;
        }
        // テンポラリ
        else if( pBAP->hookTargetIdFixed >= eSTROKE_HOOK_TARGET_TEMP_START && pBAP->hookTargetIdFixed <= eSTROKE_HOOK_TARGET_TEMP_END ){
            pal = BEZIER_WP_HOOK_TEMP_POINT_PAL + pBAP->workPathPalOfsS;
        }
        // 通常
        else{
            pal = BEZIER_WP_HOOK_POINT_PAL + pBAP->workPathPalOfsS;
        }
        ...
        // 奇数にする
        size = 2*(size/2)+1;
        PutTexForWorkPath( x0, y0, size, NULL, pal, 0, pBAP->isOfsValid, pBAP->isRefJoint );
    }
}
...
// [Work] ワークパス用：バツの出力
//----------------------------------
void CBezier::PutCrossForWorkPath( float x0, float y0, BYTE pal, bool isOption, bool isOfsValid ){
    int cut = 0;
    ...
    PutTexForWorkPath( x0, y0, s_nSizeForWorkPathCross, s_pDotForWorkPathCross, pal, cut, isOfsValid );
}
