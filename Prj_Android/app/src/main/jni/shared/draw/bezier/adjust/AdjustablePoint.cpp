...
// 調整値のクリア
//--------------------
void CAdjustablePoint::clearAdjust( void ){
...
void CAdjustablePoint::copy( CAdjustablePoint* pData ){
    ...
    m_nRateOfsX = pData->m_nRateOfsX;
    m_nRateOfsY = pData->m_nRateOfsY;
...
void CAdjustablePoint::read( CInputBuffer* pIB ){
    ...
    m_nRateOfsX = pIB->readInt16();
    m_nRateOfsY = pIB->readInt16();
...
void CAdjustablePoint::write( COutputBuffer* pOB ){
    ...
    pOB->writeInt16( (int16)m_nRateOfsX );
    pOB->writeInt16( (int16)m_nRateOfsY );
...
// RateOfsX/Y
void CAdjustablePoint::addRateOfsXY( int addX, int addY ){
    ADD_RATE( m_nRateOfsX, addX );
    ADD_RATE( m_nRateOfsY, addY );
}
...
void CAdjustablePoint::applyOfs( int rateOfsX, int rateOfsY ){
    ...
    // [m_nRateOfsX/m_nRateOfsY]には反映させない
}

// スケール
void CAdjustablePoint::applyScale( int rateScaleX, int rateScaleY ){
    float scaleX = ((float)rateScaleX)/BEZIER_SCALE_RATE;
    float scaleY = ((float)rateScaleY)/BEZIER_SCALE_RATE;
    ...
    m_nOfsMinRateXForH *= scaleX;
    m_nOfsMinRateYForH *= scaleY;
    
    m_nOfsMaxRateXForH *= scaleX;
    m_nOfsMaxRateYForH *= scaleY;
    
    m_nOfsMinRateXForV *= scaleX;
    m_nOfsMinRateYForV *= scaleY;
    
    m_nOfsMaxRateXForV *= scaleX;
    m_nOfsMaxRateYForV *= scaleY;

    // [m_eDelayType]は無視
    // [m_eDelayDepth]は無視
    m_nDelayPowerRateX *= scaleX;
    m_nDelayPowerRateY *= scaleY;
}
...
void CAdjustablePoint::applyRateScale( int rateScale ){
    float scale = ((float)rateScale)/BEZIER_SCALE_RATE;
    ...
    m_nOfsMinRateXForH *= scale;
    m_nOfsMinRateYForH *= scale;
    m_nOfsMaxRateXForH *= scale;
    m_nOfsMaxRateYForH *= scale;
    
    m_nOfsMinRateXForV *= scale;
    m_nOfsMinRateYForV *= scale;
    m_nOfsMaxRateXForV *= scale;
    m_nOfsMaxRateYForV *= scale;
    
    // [m_eDelayType]は無視
    // [m_nDelayDepth]は無視
    m_nDelayPowerRateX *= scale;
    m_nDelayPowerRateY *= scale;
}
...
void CAdjustablePoint::setEditValueMenu( CEditValueMenu* pMenu ){
    ...
    // オフセット
    pMenu->setItemAt( id++, "OFS_X", &m_nRateOfsX, eEDIT_VALUE_TYPE_INT32, BEZIER_POSITION_RATE_MIN, BEZIER_POSITION_RATE_MAX );
    pMenu->setItemAt( id++, "OFS_Y", &m_nRateOfsY, eEDIT_VALUE_TYPE_INT32, BEZIER_POSITION_RATE_MIN, BEZIER_POSITION_RATE_MAX );
