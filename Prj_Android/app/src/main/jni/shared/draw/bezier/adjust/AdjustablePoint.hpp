...
#define IMPLEMENT_TEMP_ADJUST                                                               \
    ...
    public:                                                                                 \
        ...
        inline void fixTempAdjust( int oRX, int oRY, int sRX, int sRY ){                    \
            m_nTempAdjustRateOfsX += oRX;                                                   \
            m_nTempAdjustRateOfsY += oRY;                                                   \
            m_nTempAdjustRateScaleX = m_nTempAdjustRateScaleX*sRX/BEZIER_SCALE_RATE;        \
            m_nTempAdjustRateScaleY = m_nTempAdjustRateScaleY*sRY/BEZIER_SCALE_RATE;        \
        }                                                                                   \
...
protected:
    ...
    // オフセット座標
    int m_nRateOfsX, m_nRateOfsY;                       // [2x2]: 割合オフセット座標
...
public:
    ...
    inline void setRateOfsXY( int rOfsX, int rOfsY ){ m_nRateOfsX = rOfsX; m_nRateOfsY = rOfsY; }
    ...
    inline int getRateOfsX( void ){ return( m_nRateOfsX ); }
    inline int getRateOfsY( void ){ return( m_nRateOfsY ); }
    ...
    // 判定
    inline bool isValidForH( void ){
        return( m_nOfsMinRateXForH != 0.0f || m_nOfsMaxRateXForH != 0.0f || m_nOfsMinRateYForH != 0.0f || m_nOfsMaxRateYForH != 0.0f );
    }
    
    inline bool isValidForV( void ){
        return( m_nOfsMinRateXForV != 0.0f || m_nOfsMaxRateXForV != 0.0f || m_nOfsMinRateYForV != 0.0f || m_nOfsMaxRateYForV != 0.0f );
    }

    inline bool isValidForDelay( void ){
        return( m_eDelayType != eDELAY_LOG_INVALID );
    }
    ...
    void addRateOfsXY( int addX, int addY );
