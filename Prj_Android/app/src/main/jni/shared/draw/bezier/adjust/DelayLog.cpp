...
/*+----------------------------------------------------------------+
  ...
  +----------------------------------------------------------------+*/
// バネの長さ（とりあえず）
#define DELAY_LOG_SPRING_LENGTH     80
#define DELAY_LOG_POS_RATE          24
#define DELAY_LOG_DEPTH_RANGE       8
#define DELAY_LOG_DEPTH_RANGE_POW   0.66f
...
int CDelayLog::getDelayRateX( eDELAY_LOG type, int depth ){
    ...
    if( IS_DELAY_LOG_VALID( type ) ){
        // [depth==0]は処理されていない可能性があるのでアクセスしない
        depth += 1;
        if( depth < 1 ){
            depth = 1;
        ...
        // 影響範囲
        int range = depth - DELAY_LOG_DEPTH_RANGE;
        if( range < 1 ){ range = 1; }

        // デプスによる値の影響を累積してみる
        float adj = 1.0f;
        for( int i=depth; i>=range; i-- ){
            x += adj * m_nArrArrLogX[type][i];
            adj *= DELAY_LOG_DEPTH_RANGE_POW;
        }
    }
