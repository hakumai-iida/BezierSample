...
/*+----------------------------------------------------------------+
  ...
  +----------------------------------------------------------------+*/
// 確保サイズ
#define MAX_ALLOC_PAINT_OBJECT_DATA   (8*1024)
...
// 調整値クリア
//------------
void CPaintObjectData::clearAdjust( void ){
...
void CPaintObjectData::setEditValueMenu( CEditValueMenu* pMenu ){
    // 編集項目数設定
    pMenu->setItemNum( 4 + 4 + ePOD_FLAG_MAX );
    ...
    pMenu->setItemAtAsBit( id++, "FLAG: MIRROR_COPY", &m_nFlag, eEDIT_VALUE_TYPE_INT32, ePOD_FLAG_MIRROR_COPY );
    pMenu->setItemAtAsBit( id++, "FLAG: RESET_LAST_IF_MC", &m_nFlag, eEDIT_VALUE_TYPE_INT32, ePOD_FLAG_RESET_LAST_IF_MIRROR );
