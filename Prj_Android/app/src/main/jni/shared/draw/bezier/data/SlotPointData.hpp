...
enum eSPD_FLAG{
    ...
    eSPD_FLAG_FLIP_H,               // 01:[fH] 対象のスロットをヨコ方向に反転する
    eSPD_FLAG_FLIP_V,               // 02:[fV] 対象のスロットをタテ方向に反転する
...
protected:
	...
	int m_nSlotIndex;               // [2]: インデックス（※[0=左、1=右]等、[-1]で親の値を引き継ぐ）
...
public:
    ...
    // 設定
    //------------
    void set( int rX, int rY, eBD_SLOT slot, int slotIndex=0 );
