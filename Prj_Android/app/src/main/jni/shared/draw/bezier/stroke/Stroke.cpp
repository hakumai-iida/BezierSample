...
// フックターゲット調整
//-------------------
int CStroke::AdjustHookTargetForSlotIndex( eSTROKE_HOOK_TARGET target, eBD_SLOT slot, int slotIndex ){
    int idFixed = target;
    ...
    if( IS_STROKE_HOOK_TARGET_VALID( target ) ){
        ...
        // パーツ
            ...
            if( IS_BD_SLOT_VALID( slot ) ){
                ...
                ofs += (target-eSTROKE_HOOK_TARGET_CHECK_START_FOR_PART);
                
                idFixed = eSTROKE_HOOK_TARGET_MAX + ofs;
            }
        }
        // スロットインデックスが有効（右の要素）であれば
            ...
            if( target >= eSTROKE_HOOK_TARGET_CHECK_START_FOR_FIXED && target <= eSTROKE_HOOK_TARGET_CHECK_END_FOR_FIXED ){
                idFixed = eSTROKE_HOOK_TARGET_CHECK_START_FOR_FIXED + 2*((target-eSTROKE_HOOK_TARGET_CHECK_START_FOR_FIXED)/2);
                idFixed += 1;
            }
            // 腕
            else if( target >= eSTROKE_HOOK_TARGET_CHECK_START_FOR_ARM && target <= eSTROKE_HOOK_TARGET_CHECK_END_FOR_ARM ){
                idFixed = eSTROKE_HOOK_TARGET_CHECK_START_FOR_ARM + 2*((target-eSTROKE_HOOK_TARGET_CHECK_START_FOR_ARM)/2);
                idFixed += 1;
            }
            // 脚
            else if( target >= eSTROKE_HOOK_TARGET_CHECK_START_FOR_LEG && target <= eSTROKE_HOOK_TARGET_CHECK_END_FOR_LEG ){
                idFixed = eSTROKE_HOOK_TARGET_CHECK_START_FOR_LEG + 2*((target-eSTROKE_HOOK_TARGET_CHECK_START_FOR_LEG)/2);
                idFixed += 1;
            }
        }
    }
...
// タッチターゲット調整
//-------------------
int CStroke::AdjustTouchTargetForSlotIndex( eSTROKE_TOUCH_TARGET target, int slotIndex ){
    int idFixed = target;
    ...
    if( IS_STROKE_TOUCH_TARGET_VALID( target ) ){
        ...
        // スロットインデックスが有効（右の要素）であれば
            ...
            if( target >= eSTROKE_TOUCH_TARGET_CHECK_START_FOR_ARM && target <= eSTROKE_TOUCH_TARGET_CHECK_END_FOR_ARM ){
                idFixed = eSTROKE_TOUCH_TARGET_CHECK_START_FOR_ARM + 2*((target-eSTROKE_TOUCH_TARGET_CHECK_START_FOR_ARM)/2);
                idFixed += 1;
            }
            // 脚
            else if( target >= eSTROKE_TOUCH_TARGET_CHECK_START_FOR_LEG && target <= eSTROKE_TOUCH_TARGET_CHECK_END_FOR_LEG ){
                idFixed = eSTROKE_TOUCH_TARGET_CHECK_START_FOR_LEG + 2*((target-eSTROKE_TOUCH_TARGET_CHECK_START_FOR_LEG)/2);
                idFixed += 1;
            }
        }
    }
...
// ガイドターゲット調整
//------------------
int CStroke::AdjustGuideTargetForSlotIndex( eSTROKE_GUIDE_TARGET target, int slotIndex ){
    int idFixed = target;
    ...
    if( IS_STROKE_GUIDE_TARGET_VALID( target ) ){
        ...
        // スロットインデックスが有効（右の要素）であれば
            ...
            if( target >= eSTROKE_GUIDE_TARGET_CHECK_START_FOR_ARM && target <= eSTROKE_GUIDE_TARGET_CHECK_END_FOR_ARM ){
                idFixed = eSTROKE_GUIDE_TARGET_CHECK_START_FOR_ARM + 2*((target-eSTROKE_GUIDE_TARGET_CHECK_START_FOR_ARM)/2);
                idFixed += 1;
            }
            // 脚
            else if( target >= eSTROKE_GUIDE_TARGET_CHECK_START_FOR_LEG && target <= eSTROKE_GUIDE_TARGET_CHECK_END_FOR_LEG ){
                idFixed = eSTROKE_GUIDE_TARGET_CHECK_START_FOR_LEG + 2*((target-eSTROKE_GUIDE_TARGET_CHECK_START_FOR_LEG)/2);
                idFixed += 1;
            }
        }
    }
