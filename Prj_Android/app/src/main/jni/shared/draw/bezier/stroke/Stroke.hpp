...
class CStroke{
public:
    // 調整：返値は[int]型となる（※調整後の[target]は[eSTROKE_***_TARGET_MAX]とは別管理となるので）
    static int AdjustHookTargetForSlotIndex( eSTROKE_HOOK_TARGET target, eBD_SLOT slot, int slotIndex );
    static int AdjustTouchTargetForSlotIndex( eSTROKE_TOUCH_TARGET target, int slotIndex );
    static int AdjustGuideTargetForSlotIndex( eSTROKE_GUIDE_TARGET target, int slotIndex );
