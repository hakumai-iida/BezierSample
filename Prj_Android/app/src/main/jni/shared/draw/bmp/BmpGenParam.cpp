...
stBMP_GEN_PARAM_CELL* CBmpGenParam::addCell( stBD_PART_SEARCH_PARAM* pParam, int slotIndex, float adjForT, float adjForS ){
    ...
    if( (pPart->checkFlag( eBDPD_FLAG_REF_DRAW ) || pPart->checkFlag(eBDPD_FLAG_REF_SLOT)) ){
        ...
        // サブID設定（※無効なら０を流用＝参照元の値を流用すると循環してしまうので）
    ...
    // 用心（※セルのスロットとデータのスロットが合致しないのは困る）
    if( pCell->pData != NULL ){
        if( pCell->pData->getSlot() != pCell->slot ){
            LOGE( "@ UNMATCH DATA SLOT: pData->getSlot(%d) != pCell->slot(%d)\n", pCell->pData->getSlot(), pCell->slot );
        }
    }
    
    if( pCell->pDataRef != NULL ){
        if( pCell->pDataRef->getSlot() != pCell->slot ){
            LOGE( "@ UNMATCH REF SLOT: pDataRef->getSlot(%d) != pCell->slot(%d)\n", pCell->pDataRef->getSlot(), pCell->slot );
        }
    }
