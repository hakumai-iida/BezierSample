...
// ラベル：オーダータイプ
//----------------------
const char* g_pArrLabelLayerOrderType[] = {
    "FRONT",
    "NORMAL",
    "BACK",
...
// ラベル：オーダータイプ（短縮形）
//--------------------------
const char* g_pArrLabelLayerOrderTypeS[] = {
    "F",
    "N",
    "B",
...
void CLayerOrderCell::clear( void ){
    ...
    for( int i=0; i<eLAYER_ORDER_TYPE_MAX; i++ ){
        ...
        while( pHead != NULL ){
            stLAYER_ORDER_CELL_NODE* pNext = (stLAYER_ORDER_CELL_NODE*)pHead->pNext;
            SAFE_FREE( pHead );
            pHead = pNext;
        }
...
// ヘッダ取得
//----------------
stLAYER_ORDER_CELL_NODE* CLayerOrderCell::getNodeHead( eLAYER_ORDER_TYPE type ){
...
//----------------------------------------------
stLAYER_ORDER_CELL_NODE* CLayerOrderCell::addNode( CLayerData* pData, eLAYER_ORDER_TYPE type ){
    // 無効は無視
    if( pData == NULL ){
    ...
    // 空でなければ末尾へ追加
        ...
        while( pHead != NULL ){
            ...
            pHead = (stLAYER_ORDER_CELL_NODE*)pHead->pNext;
        }
