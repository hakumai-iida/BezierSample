...
enum eSTROKE_HOOK_TARGET{
    ...
    //-----------------------------------------------------------------------
    // パーツ枠（※パーツ毎に確保される枠／レイヤー内でオーダーが連続しない様相にはこれを使う）
    //-----------------------------------------------------------------------
    ...
    //-----------------------------------------------------------------------
    // 特定枠：（※パーツをまたいで指定される要素／データ作成の仕様上で用途が限定できるもの）
    //-----------------------------------------------------------------------
    ...
    //-----------------------------------------------------------------------
    // 特殊枠：フリル用（※フリルの処理中にのみ利用される／データへの指定は不可）
    //-----------------------------------------------------------------------
...
// 左右の区別をする範囲：特定
#define eSTROKE_HOOK_TARGET_CHECK_START_FOR_FIXED   eSTROKE_HOOK_TARGET_HEAD_TOP_L
#define eSTROKE_HOOK_TARGET_CHECK_END_FOR_FIXED     eSTROKE_HOOK_TARGET_ARM_TO_BACK_R
...
enum eSTROKE_TOUCH_TARGET{
    ...
    //-------------------------------------------------------------------
    // 顔（※[FcBase]から[FcHairline]（生際から顎にかけてのライン）を左右で指定する想定）
    ...
    //-------------------------------------------------------------------
    // 固定枠：半身（※パーツ単位での組み替えがないので汎用的な枠でもつ）
    //-------------------------------------------------------------------
